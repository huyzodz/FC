...
#define BMI270_RAW_2_U16(msb, lsb) \
            ((int16_t)(((int16_t)(msb) << 8) | (lsb)))





volatile static imu_data_t bias;

volatile uint8_t bmi270_size_read;

// config file for bmi270 ~8Kb
...
int bmi270_mode_low_power(void)
{
    ...
    if(i2c_write_reg(ADDRESS_DEVICE_BMI270, 0x7D, 0x04, NUM_I2C_BMI270) == -1)
        return -1;
...
int bmi270_mode_normal_power(void)
{
    ...
    // set acc_od to 100Hz
    if(i2c_write_reg(ADDRESS_DEVICE_BMI270, 0x40, 0xA8, NUM_I2C_BMI270) == -1)
        return -1;
    ...
    // leave fifo_Self_wakeup enabble
    if(i2c_write_reg(ADDRESS_DEVICE_BMI270, 0x7C, 0x02, NUM_I2C_BMI270) == -1)
        return -1;
...
int bmi270_init(bmi270_power_mode_t mode)
{
    ...
    i2c_config_t cfg = {
        ...
        .dma_read = CHANNEL_DMA_READ_BMI270,
    ...
    // init bmi270 base on document 

    // one mean error
    while (1)
    {
        // check id device
        if (i2c_read_reg(ADDRESS_DEVICE_BMI270, 0x00, NUM_I2C_BMI270) != 0x24)
            continue;
        delay_ms(1);
        // disable PWR_CONF.adv_power_save
        if (i2c_write_reg(ADDRESS_DEVICE_BMI270, 0x7C, 0x00, NUM_I2C_BMI270) == -1)
            continue;
        delay_ms(2);
        
        //prepare config load INIT_CTRL = 0x00
        if (i2c_write_reg(ADDRESS_DEVICE_BMI270, 0x59, 0x00, NUM_I2C_BMI270))
            continue;
        delay_ms(1);
        // load config file to bmi270
        if (i2c_burst_write(ADDRESS_DEVICE_BMI270, 0x5E, sizeof(bmi270_config_file), NUM_I2C_BMI270, bmi270_config_file) == -1)
            continue;
        delay_ms(1);
    ...
    while (1)
    {
        if (mode == BMI270_LOW_POWER_MODE && (bmi270_mode_low_power() == -1))
            continue;
        else if (mode == BMI270_NORMAL_POWER_MODE && (bmi270_mode_normal_power() == -1))
            continue;
        else if (mode == BMI270_HIGH_POWER_MODE && (bmi270_mode_high_power() == -1))
            continue;
...
void bmi270_calib()
{
    ...
    bias.gyroy = (float)sum_gy/BIAS_SAMPLE;
    bias.gyroz = (float)sum_gz/BIAS_SAMPLE;

    bias.accx = (float)sum_ax/BIAS_SAMPLE;
...
/*
    wait_read_done = 0 if do not want to wait
    =1 if watit
    this func read raw data from bmi270
*/
